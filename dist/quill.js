  ...
  register(path, target, overwrite = false) {
    if (typeof path !== 'string') {
      ...
      if (typeof target.register === 'function') {
        target.register(this.quill.editorRegistry);
      }
    }
  }
...
function endsWithNewlineChange(delta, editorInstance) {
  ...
  if (lastOp.attributes != null) {
    return Object.keys(lastOp.attributes).some(attr => {
      return editorInstance.query(attr, _parchment2.default.Scope.BLOCK) != null;
    });
  }
  return false;
}

function getLastChangeIndex(delta, editorInstance) {
  const deleteLength = delta.reduce((length, op) => {
    return length + (op.delete || 0);
  }, 0);
  let changeIndex = delta.length() - deleteLength;
  if (endsWithNewlineChange(delta, editorInstance)) {
    changeIndex -= 1;
  }
  return changeIndex;
}
...
_core2.default.QUILL_EXTRA_DEFAULTS = {
...
class ListItem extends _block2.default {
  static formats(domNode, editorRegistry) {
    return domNode.tagName === this.tagName ? undefined : super.formats(domNode, editorRegistry);
  }
